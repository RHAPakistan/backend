...
module.exports = {
    ...
    search_requests: expressAsyncHandler(async(req, res)=>{
        try{
            let requests = await InductionRequest.find({$text: { $search: req.body.text }});
            if(requests){   
                if(requests.length >0){
                        res.status(200).send({error: 0, requests: requests})
                    }
                else{
                    requests = await InductionRequest.find();
                    res.send({error: 1, message: "No such result. Returning all data", requests: requests});
                }
            }
            else{
                res.status(401).send({error: 2, message: "Requests not found"})
            }
        }
        catch (error){
